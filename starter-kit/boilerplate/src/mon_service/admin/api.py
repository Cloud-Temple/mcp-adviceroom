...
async def _api_whoami(send, token):
    ...
    settings = get_settings()
    auth_type = "bootstrap" if hmac.compare_digest(token, settings.admin_bootstrap_key) else "token"
...
def _is_admin(token: str) -> bool:
    ...
    if not token:
        return False
    settings = get_settings()
    # ⚠️ Comparaison constante contre timing attacks (side-channel)
...
async def _json_response(send, status, data):
    """Envoie une réponse JSON (CORS restreint à same-origin)."""
    # ⚠️ Pas de Access-Control-Allow-Origin: * — l'admin est servi
    # par le même domaine que l'API (same-origin), pas besoin de CORS.
    # Un wildcard * exposerait l'API admin à des requêtes cross-origin.
