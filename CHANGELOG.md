...
**Politique de divulgation** : ajout d'un `SECURITY.md` (FR/EN). Le dépôt est public et n'offrait aucun canal privé de signalement — quiconque trouvait une faille n'avait d'autre choix que d'ouvrir une issue publique, exposant tous les déploiements jusqu'au correctif. Le signalement privé GitHub est activé, et le document précise le périmètre, les délais de réponse et ce qui n'est pas traité comme une vulnérabilité.

Correction des findings **HIGH #3** et **#4** de l'audit du 24/08/2026.

- **Secrets exposés par la représentation des settings** (`config/settings.py`) : les 8 secrets étaient typés `str` nu, et `repr(settings)` les imprimait **en clair** — une trace d'exception, un log de debug ou un dump de configuration suffisait à les fuiter. Tous passent en `pydantic.SecretStr`, qui affiche `**********` et impose `.get_secret_value()`, rendant chaque lecture de secret visible à la relecture. Portée à connaître : cela protège la représentation des settings, **pas** l'environnement du processus — les providers LLM lisent encore leurs clés via `os.getenv()`, donc un dump d'environnement resterait une fuite. C'est une atténuation, pas une élimination
- **XSS dans le fichier HTML exporté** (`static/admin.html`, `mdExport()`) : trois défauts distincts. Une substitution réinjectait des guillemets bruts **juste après** l'échappement, l'annulant — défaut non mentionné dans l'audit et qui neutralisait le durcissement d'`esc()`. L'URL des liens était interpolée telle quelle dans `href`, laissant passer `javascript:`, qu'échapper les chevrons ne bloque pas. Et les liens n'avaient pas de `rel="noopener"`, alors que le fichier exporté s'ouvre hors de toute origine de confiance. Les URL passent désormais par un `safeHref()` à **liste d'autorisation** (`http`, `https`, `mailto`, relatifs et ancres), insensible à la casse et aux caractères de contrôle — `java\tscript:` étant interprété comme `javascript:` par les navigateurs. Une URL refusée rend le libellé en texte simple
